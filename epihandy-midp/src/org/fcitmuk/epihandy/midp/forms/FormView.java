...
public class FormView extends AbstractView implements AlertMessageListener {
	...
	public void handleListSelectCommand(Displayable d){
		...
		if(currentQuestion.getDef().isEnabled() && (currentQuestion.getDef().isLocked() == false))//allow only fields which are not locked and enabled
		{
			...
			//separate handling for GPS Questions - Omar 11-7-11
			if(edit) {
				if (currentQuestion.getDef().getType() == QuestionDef.QTN_TYPE_GPS) {
					//if no Questions 					
					if (formData.getDef().getQuestionByName("Longitude") == null
							&& formData.getDef().getQuestionByName("Latitude") == null) {
						return;
					}
				}
			}
