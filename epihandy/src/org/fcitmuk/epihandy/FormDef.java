...
public class FormDef implements Persistent{
	...
	/**
	 * Gets a QuestionDef by its Text property
	 *
	 * @param String representing the Test property of QuestionDef 
	 * @param regerence to QuestionDef object
	 */
	public QuestionDef getQuestionByName(String name)
	{
		...
		for(byte i=0; i<getPages().size(); i++){
			QuestionDef def = ((PageDef)getPages().elementAt(i)).getQuestionByName(name);
			if(def != null)
				return def;
		}
	...
	public void addQuestion(QuestionDef qtn){
		if(pages.size() == 0){
			PageDef page = new PageDef(/*this.getVariableName()*/"Page1",Byte.parseByte("1"),new Vector());
	...
	public void addPage(){
		pages.addElement(new PageDef("Page"+pages.size(),(byte)pages.size(),new Vector()));
	}
